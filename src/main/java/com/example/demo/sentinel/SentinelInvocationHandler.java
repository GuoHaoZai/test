	...
	public Object invoke(final Object proxy, final Method method, final Object[] args)
		...
		if (target instanceof Target.HardCodedTarget) {
			...
			if (methodMetadata == null) {
				...
				Entry entry = null;
				try {
					Request.Options options = new Request.Options(2000L, TimeUnit.MILLISECONDS, 1L, TimeUnit.MILLISECONDS, true);
					Object[] newArgs = Arrays.copyOf(args, args.length + 1);
					newArgs[args.length] = options;
					ContextUtil.enter(resourceName);
					entry = SphU.entry(resourceName, EntryType.OUT, 1, newArgs);
					result = methodHandler.invoke(newArgs);
				} catch (Throwable ex) {
					// fallback handle
